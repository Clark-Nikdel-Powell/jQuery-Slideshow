...
$.fn.slideshow = function(opts) {
	...
	var animations = {
		...
		slide: {
			...
			forward: function($newSlide, $oldSlide, $slides) {
				$oldSlide.animate(this.leftNegative , options.speed, function() {
					$oldSlide.removeClass(options.currentClass).css(this.leftPositive).appendTo($slides);
				});
				$newSlide.animate(this.leftZero, options.speed, function() {
					$newSlide.addClass(options.currentClass);
				});
			},

			backward: function($newSlide, $oldSlide, $slides) {
				$oldSlide.animate(this.leftPositive , options.speed, function() {
					$oldSlide.removeClass(options.currentClass);
				});
				$newSlide.css(this.leftNegative).animate(this.leftZero, options.speed, function() {
					$newSlide.addClass(options.currentClass);
				});
			}
	...
	$this.on('click', options.nextButton, function(e) {
		clickEvent(e, function(){
			if ($(options.slides + ' *').is(':animated')) {return false;}
